...
def proc_func(queue, event_queue, return_dict, **kwargs):
    from webview import Window
    from webview.platforms import cocoa as guilib
    from threading import Thread
    
    guilib.__dict__.update(kwargs)
...
p = mp.Process(target=proc_func,
               args=(queue, event_queue, return_dict),
               kwargs={
                   '_debug': guilib._debug,
                   '_user_agent': guilib._user_agent,
                   'OPEN_DIALOG': guilib.OPEN_DIALOG,
                   'FOLDER_DIALOG': guilib.FOLDER_DIALOG,
                   'SAVE_DIALOG': guilib.SAVE_DIALOG
               })
p.start()
