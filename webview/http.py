...
class SSLWSGIRefServer(bottle.ServerAdapter):
    def run(self, handler: WSGIApplication) -> None:  # pragma: no cover
        ...
        # The cert chain has been loaded into the SSL context, so the temporary
        # key/cert files on disk are no longer needed and are removed.
        if os.path.exists(self.pywebview_keyfile):
            os.unlink(self.pywebview_keyfile)
        if os.path.exists(self.pywebview_certfile):
            os.unlink(self.pywebview_certfile)
        try:
