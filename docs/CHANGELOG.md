...
- `Android` Fix `window.destroy()`, which hung instead of closing the window. `destroy_window()` took no `uid` while the rest of the API passes one, and the destroy event was dispatched without the activity argument its handler expects - two `TypeError`s on the same path, both swallowed by a bare `except`. The window is now also removed from `webview.windows` as it closes, so a window created after a destroyed one is the one that gets launched.
