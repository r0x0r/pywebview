...
- APIs whose result is produced asynchronously cannot be delivered while the handler holds the GUI thread. Rather than block forever, these raise `webview.errors.ReentrantCallError` (a subclass of `RuntimeError`) on the affected backends: `window.evaluate_js()` on Cocoa, GTK, WinForms/EdgeChromium, WinUI3, Android and Qt when using QtWebEngine; and `window.get_cookies()` on all but Qt, which answers from its own cookie cache. QtWebKit's script evaluation is synchronous and returns its result inline instead of raising, as do CEF (its JS results arrive on a dedicated CEF thread, not the GUI thread) and MSHTML (`InvokeScript` is synchronous).
